...
namespace HNLib.Dependencies
{
    ...
    {
        public event Action OnInjectEnd;
        
        private const BindingFlags _bindingFlags =
            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;

        //의존성 주입을 위해 가지고 있는 Provide 객체들
        private readonly List<MonoBehaviour> _excludeProviders = new List<MonoBehaviour>();
        ...
        private void Inject(MonoBehaviour injectable)
        {
            ...
            IEnumerable<FieldInfo> injectableFields = type.GetFields(_bindingFlags)
                .Where(f => Attribute.IsDefined(f, typeof(InjectAttribute)) && f.GetValue(injectable) == null);
        ...
        private void RegisterProvider(IDependencyProvider provider)
        {
            ...
            {
                _registry.TryAdd(provider.GetType(), provider);
                return;
            }
        ...
        private IEnumerable<MonoBehaviour> FindMonoBehaviours()
        {
            return FindObjectsByType<MonoBehaviour>(FindObjectsSortMode.None)
                .Where(mono => _excludeProviders.Contains(mono) == false); //정렬없이 모든 모노 가져오기
        }

        public void AddExcludeProvider(MonoBehaviour provider)
        {
            _excludeProviders.Add(provider);
        }
    }
}